			...
			switch (key)
			{
				...
				case 'o':
					if(access(arg, F_OK) == -1)
						outcsv_writeheader = 1;
					outcsv = fopen(arg, "a+");
					if (!outcsv) {
						fprintf(stderr, "Error opening output CSV file %s\n\n", arg);
						return ARGP_ERR_UNKNOWN;
					}
					if (outcsv_writeheader)
						fprintf(outcsv, "time,frequency_mhz,width_khz,peak,diff\n");
						fflush(outcsv);
					break;
				case ARGP_KEY_ARG:
					if (state->arg_num > 0){
...
argp_option Arguments::options[] = {
	...
	{"output-csv", 'o', "OUTCSV", 0, "Output results to CSV file"},
	{0}
};
...
"Copyright (C) 2015  Laurent Ghigonis <laurent@gouloum.fr>\n"
"Copyright (C) 2012  Nicholas Tomlinson\n"
"\n"
"This program is free software: you can redistribute it and/or modify\n"
"it under the terms of the GNU General Public License as published by\n"
